...
@api_router.post("/send-password-reset-email")
async def send_password_reset_email(request: SendPasswordResetEmailRequest):
    """Send password reset email using Gmail API with Firebase reset link"""
    try:
        from email.mime.text import MIMEText
        from google.oauth2.credentials import Credentials
        from google.auth.transport.requests import Request
        ...
        try:
            reset_link = admin_auth.generate_password_reset_link(request.to_email)
        except Exception as e:
            logging.error(f"Firebase password reset link generation failed: {e}")
            return {"success": False, "message": f"Failed to generate reset link: {str(e)}"}
...
We received a request to reset your password for your AdmitAI account.
...
AdmitAI Team
        ...
        message['to'] = request.to_email
        message['from'] = os.environ.get('GMAIL_FROM_EMAIL', 'noreply@admitai.com')
        message['subject'] = "Reset Your AdmitAI Password"
        
        # Send email via Gmail API
        raw = base64.urlsafe_b64encode(message.as_bytes()).decode()
        service.users().messages().send(userId="me", body={'raw': raw}).execute()
        
        logging.info(f"✅ Password reset email sent to {request.to_email}")
